...
extern void F77_NAME(fit_cvam_model)(int *model_type_int, int *method_int, int *dim_vec, int *input_data, int *input_data_freq_int, int *n_levels_matrix, int *packed_map, double *model_matrix, double *offset, int *str_zero_int, double *flatten, double *ridge, int *prior_data, int *prior_data_freq_int, double *ctrl_real, int *ctrl_int, int *omit_data_int, int *dim_vec_est, int *estimate_info, int *estimate_var_info, int *ctrl_mcmc_int, double *ctrl_mcmc_real, int *dim_vec_mcmc, double *prob, double *beta, double *beta_hat, double *vhat_beta_rwm, int *iter, int *converged, double *max_diff, double *loglik, double *logP, double *lambda, double *freq, double *freq_mean, int *freq_int, double *score, double *vhat_beta, double *prob_mean, double *beta_mean, double *beta_cov_mat, double *total_freq_use_prior, int *total_freq_use_data, int *degrees_of_freedom, double *packed_estimates, double *packed_estimates_mean, double *packed_SEs, double *beta_series, double *prob_series, double *logp_series, int *imputed_freq_int, double *packed_estimates_series, int *n_iter_actual, int *n_sample_actual, int *n_imp_actual, double *mh_accept_rate, double *start_logP, int *status, int *msg_len_max, int *msg_codes, int *msg_len_actual);
...
static const R_FortranMethodDef FortranEntries[] = {
    ...
    {"fit_cvam_model",        (DL_FUNC) &F77_NAME(fit_cvam_model),        61},
    {NULL, NULL, 0}
};
