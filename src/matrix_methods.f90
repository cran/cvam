...
800   call err_handle(err, 1, &
  ...
  !####################################################################
   integer(kind=our_int) function kronecker_symm(a, b, ab, &
      ...
      integer(kind=our_int) :: na, nb, ia, ja, ib, jb, i, j, ii, jj
      ! begin
      answer = RETURN_FAIL
      na = size( a, 1 )
      if( size( a, 2 ) /= na ) goto 200
      nb = size( b, 1 )
      if( size( b, 2 ) /= nb ) goto 200
      if( size( ab, 1 ) /= size( ab, 2 ) ) goto 200
      if( size( ab, 1 ) /= na * nb ) goto 300
...
300   call err_handle(err, 1, &
  ...
  !####################################################################
   integer(kind=our_int) function kronecker_lower_tri(a, b, ab, &
      ...
      integer(kind=our_int) :: na, nb, ia, ja, ib, jb, i, j
      ! begin
      answer = RETURN_FAIL
      na = size( a, 1 )
      if( size( a, 2 ) /= na ) goto 200
...
300   call err_handle(err, 1, &
  ...
  !####################################################################
   integer(kind=our_int) function kronecker(a, b, ab, err) &
      ...
      integer(kind=our_int) :: ma, na, mb, nb, &
           ia, ja, ib, jb, i, j
      ! begin
      answer = RETURN_FAIL
      ma = size( a, 1 )
      ...
      mb = size( b, 1 )
      nb = size( b, 2 )
      if( size( ab, 1 ) /= ma * mb ) goto 300
      if( size( ab, 2 ) /= na * nb ) goto 300
      do ia = 1, ma
         do ja = 1, na
            i = ( ia - 1 ) * mb
            do ib = 1, mb
